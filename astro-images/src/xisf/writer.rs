...
pub fn wcs_to_xisf_properties(wcs_keywords: &[astro_wcs::WcsKeyword]) -> Vec<XisfProperty> {
    ...
    for kw in wcs_keywords {
        match kw.name.as_str() {
            "CTYPE1" => {
                if let astro_wcs::WcsKeywordValue::String(s) = &kw.value {
                    proj_code = extract_projection_code(s);
                }
            }
            "CRVAL1" => {
                if let astro_wcs::WcsKeywordValue::Real(v) = &kw.value {
                    crval1 = Some(*v);
                }
            }
            "CRVAL2" => {
