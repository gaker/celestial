...
impl KeywordBuilder {
    ...
    pub fn date_from_utc(&mut self, utc: &astro_time::UTC) -> &mut Self {
        self.date(utc.to_iso8601())
    }

    pub fn date_obs_from_utc(&mut self, utc: &astro_time::UTC) -> &mut Self {
...
mod tests {
    ...
    fn keyword_real_creates_real_keyword() {
        let keyword = Keyword::real("CRVAL1", astro_core::constants::PI);

        assert_eq!(keyword.name, "CRVAL1");
        assert_eq!(
            keyword.value,
            Some(KeywordValue::Real(astro_core::constants::PI))
        );
        assert_eq!(keyword.comment, None);
    }
    ...
    fn keyword_value_as_integer_returns_none_for_other_types() {
        ...
        let value = KeywordValue::Real(astro_core::constants::PI);
        assert_eq!(value.as_integer(), None);
    }
    ...
    fn keyword_value_as_real_returns_real() {
        let value = KeywordValue::Real(astro_core::constants::PI);
        assert_eq!(value.as_real(), Some(astro_core::constants::PI));
    ...
    fn keyword_value_display_real() {
        ...
        assert_eq!(
            format!("{}", value),
            format!("{}", astro_core::constants::PI)
        );
    ...
    fn keyword_value_display_complex() {
        ...
        let value = KeywordValue::Complex(-1.5, astro_core::constants::PI);
        assert_eq!(
            format!("{}", value),
            format!("(-1.5, {})", astro_core::constants::PI)
        );
