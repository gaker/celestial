...
impl Command for Fix {
    ...
    fn description(&self) -> &str { "Fix terms at current values during fit" }
    ...
    fn execute(&self, session: &mut Session, args: &[&str]) -> Result<CommandOutput> {
        ...
        if args[0].eq_ignore_ascii_case("ALL") {
            ...
            return Ok(CommandOutput::Text(format!(
                "Fixed all {} terms", session.model.term_count()
            )));
        }
...
impl Command for Unfix {
    fn name(&self) -> &str { "UNFIX" }
    fn description(&self) -> &str { "Allow terms to be fitted" }
    ...
    fn execute(&self, session: &mut Session, args: &[&str]) -> Result<CommandOutput> {
        ...
        if args[0].eq_ignore_ascii_case("ALL") {
            ...
            return Ok(CommandOutput::Text(format!(
                "Unfixed all {} terms", session.model.term_count()
            )));
        }
        ...
        Ok(CommandOutput::Text(format!("Unfixed: {}", unfixed.join(" "))))
    }
}
