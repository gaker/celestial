...
impl Command for Fauto {
    fn name(&self) -> &str { "FAUTO" }
    fn description(&self) -> &str { "Auto-add harmonics up to Nth order" }

    fn execute(&self, session: &mut Session, args: &[&str]) -> Result<CommandOutput> {
        if args.is_empty() {
            return Err(Error::Parse("FAUTO requires a harmonic order".into()));
        }
        let order: usize = args[0].parse()
        ...
        if added.is_empty() {
            Ok(CommandOutput::Text(format!(
                "All harmonics up to order {} already in model", order
            )))
        } else {
            Ok(CommandOutput::Text(format!(
                "Added {} harmonics: {}", added.len(), added.join(" ")
            )))
        }
    }
}
