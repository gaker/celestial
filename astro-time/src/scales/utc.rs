...
//!
//! ```
//! use astro_time::{JulianDate, UTC};
//! use astro_time::scales::utc::utc_from_calendar;
//!
//! // From Unix timestamp
...
impl UTC {
    ...
    pub fn new(seconds: i64, nanos: u32) -> Self {
        let days = seconds / astro_core::constants::SECONDS_PER_DAY;
        let remainder_seconds = seconds % astro_core::constants::SECONDS_PER_DAY;
        let jd1 = UNIX_EPOCH_JD + days as f64;
        let jd2 = (remainder_seconds as f64
            + nanos as f64 / astro_core::constants::NANOSECONDS_PER_SECOND_F64)
            / SECONDS_PER_DAY_F64;
...
mod tests {
    ...
    fn test_utc_new_precision_preservation() {
        let seconds_50_years = 50 * 365 * astro_core::constants::SECONDS_PER_DAY as u32;
        let nanos = 123456789u32;

        let utc = UTC::new(seconds_50_years as i64, nanos);
        let jd = utc.to_julian_date();

        let expected_days = seconds_50_years / astro_core::constants::SECONDS_PER_DAY as u32;
        let remainder_secs = seconds_50_years % astro_core::constants::SECONDS_PER_DAY as u32;
