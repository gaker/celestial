...
use astro_core::{matrix::RotationMatrix3, Angle};
use astro_time::{transforms::PrecessionCalculator, TT};
...
impl EclipticPosition {
    ...
    pub fn mean_obliquity(&self) -> Angle {
        ...
        Angle::from_radians(astro_core::obliquity::iau_2006_mean_obliquity(
            jd.jd1(),
            jd.jd2(),
        ))
    }
    ...
    pub fn true_obliquity(&self) -> CoordResult<Angle> {
        use astro_time::transforms::nutation::NutationCalculator;
    ...
    pub fn angular_separation(&self, other: &Self) -> Angle {
        ...
        let angle_rad = astro_core::math::vincenty_angular_separation(
            sin_b1,
            cos_b1,
            sin_b2,
            cos_b2,
...
impl CoordinateFrame for EclipticPosition {
    fn to_icrs(&self, epoch: &TT) -> CoordResult<ICRSPosition> {
        ...
        let d2pi = astro_core::constants::TWOPI;
        let mut ra_normalized = ra % d2pi;
        if ra_normalized < 0.0 {
            ra_normalized += d2pi;
        }

        let dpi = astro_core::constants::PI;
